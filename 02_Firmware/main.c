/**
 * @file main.c
 * @brief H2_Control_Board dsPIC33CK512MP710 Main Source File with Integrated BOP Control & Modbus
 * 
 * @note 본 메인 소스는 시스템 설계 사양에 맞춰 다음 통신 및 제어 스케줄러를 가동합니다:
 *       1. 시스템 하드웨어 핀 및 GPIO 초기화
 *       2. ADS1115(16ch ADC), DAC60516(12ch DAC), MAX31856(다채널 TC) 장치 드라이버 초기화
 *       3. RS-485 및 RS-422 UART 통신 개시
 *       4. 주기적 센서 계측(온도, 압력, 유량 등) 결과의 Modbus 슬레이브 데이터베이스 바인딩
...
int main(void)
{
    ...
    while (1)
    {
        ...
        // 노트북에서 레지스터 0~11번지를 수정 시 DAC60516을 즉각 하드웨어 동기화합니다.
        Modbus_Task();

        // [3] 주기적 아날로그 센서 및 열전대 온도 스캔 연동 (약 200ms 주기로 스케줄링)
        sensor_scan_counter++;
        if (sensor_scan_counter >= 30000UL) // 메인 루프 기준 시간 지연 분주
        {
            ...
            // 소수점 1자리 정밀도를 보존하기 위해 온도를 10배 곱한 정수로 레지스터에 기록합니다.
            // 예: 25.4도는 254로 전송되어 관제 프로그램에서 10.0으로 나누어 소수 표현.
            for (uint8_t ch = 0; ch < TC_MAX_CHANNELS; ch++)
            {
                // Enum 번지 상 빈 채널(CH0, CH20)은 제외
                if (ch == TC_CH0_UNUSED || ch == TC_CH20_UNUSED)
                {
                    continue;
                }
            ...
            // B. 16채널 ADS1115 ADC 전압 계측 및 Modbus DB 바인딩 (input_regs 32 ~ 47)
            // 4개의 ADS1115 칩셋(0x48~0x4B)을 순차 리드하여 16비트 원시 ADC 코드를 입력 레지스터에 대입합니다.
            ...
            // B-2. DS3231 RTC 실시간 시각 획득 및 Modbus DB 바인딩 (input_regs 50 ~ 55)
            ...
            // C. 디지털 입력 포트(DI) 비트 상태를 Modbus Discrete Inputs DB에 업데이트
            // 0xFFFF 리셋 후 pin_map.h 입력 상태 비트 팩킹
            uint16_t di_pack = 0;
            if (DI_HT193_WD_PORT)  di_pack |= (1 << 0);
            if (DI_HT194_WD_PORT)  di_pack |= (1 << 1);
            if (DI_HT195_WD_PORT)  di_pack |= (1 << 2);
            if (DI_HT196_WD_PORT)  di_pack |= (1 << 3);
            if (DI_FD176_PORT)     di_pack |= (1 << 4);
            if (DI_FAN504_PORT)    di_pack |= (1 << 5);
            if (DI_GD501_PORT)     di_pack |= (1 << 6);
            if (DI_GD502_PORT)     di_pack |= (1 << 7);
            if (POWER_FAIL_DET_PORT == 0) di_pack |= (1 << 8); // 정전 감지 시 비트 셋

            modbus_db.discrete_inputs[0] = (uint8_t)(di_pack & 0xFF);
            modbus_db.discrete_inputs[1] = (uint8_t)((di_pack >> 8) & 0xFF);

            // D. Modbus Coils DB 상태를 pin_map.h 디지털 출력(DO) 포트에 물리적 연동
            // 노트북 관제단에서 코일 상태를 쓰면 MCU의 실제 핀 릴레이를 스위칭합니다.
            uint16_t do_pack = ((uint16_t)modbus_db.coils[1] << 8) | modbus_db.coils[0];
            
            DO_MC_SW_LAT   = (do_pack & (1 << 0)) ? 1 : 0;
            DO_HT193_LAT   = (do_pack & (1 << 1)) ? 1 : 0;
            DO_HT194_LAT   = (do_pack & (1 << 2)) ? 1 : 0;
            DO_HT195_LAT   = (do_pack & (1 << 3)) ? 1 : 0;
            DO_HT196_LAT   = (do_pack & (1 << 4)) ? 1 : 0;
            DO_FAN504_LAT  = (do_pack & (1 << 5)) ? 1 : 0;
            DO_LCD500_LAT  = (do_pack & (1 << 6)) ? 1 : 0;
            DO_IGN175_LAT  = (do_pack & (1 << 7)) ? 1 : 0;
        }
